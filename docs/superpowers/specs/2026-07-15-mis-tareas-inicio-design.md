...
- **Datos**: un `useTasks({ assigneeId: user.id })` (hook existente `frontend/src/hooks/useTasks.ts`). Es una query de React Query independiente del resumen del dashboard (`/dashboard/summary` no se toca).
...
- **Cada fila**: título + `empresa · proyecto` (como en "Próximos vencimientos"), con `PriorityBadge` y la fecha de vencimiento (en rojo si está vencida, usando el helper `isOverdue`/`formatDate` ya usados en el dashboard). Al hacer clic, navega a `/tareas?tarea=<id>` (la página de tareas ya abre el panel de la tarea vía el query param `tarea`).
...
1. **Ruta de aterrizaje** (`frontend/src/lib/permissions.ts`): `landingPath()` para no-admin pasa de `/proyectos` a `/tareas`. El CEO/ADMIN sigue aterrizando en el dashboard (`/`). Esto afecta el redirect de inicio de sesión y el de `RequireAdmin` (que ya usa `landingPath`).
